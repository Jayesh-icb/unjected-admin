...
const Main = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const allUsersData = response.data["LOGIN SUCCESS FOR ADMIN"]
        const activeCount = allUsersData.filter(
          (user) => !user.deactivated
        ).length
        const deletedCount = allUsersData.filter(
          (user) => user.deactivated
        ).length
    ...
    <div className="px-[25px] pt-[25px] bg-[#F8F9FC] pb-[40px]">
      ...
      <div className="grid grid-cols-4 gap-[30px] mt-[25px] pb-[15px]">
        ...
        <div className=" h-[100px] rounded-[8px] bg-white border-l-[4px] border-[#F6C23E] flex items-center justify-between px-[30px] cursor-pointer hover:shadow-lg transform hover:scale-[103%] transition duration-300 ease-out">
          <div>
            <h2 className="text-[#1cc88a] text-[11px] leading-[17px] font-bold"></h2>
            <h1 className="text-[20px] leading-[24px] font-bold text-[#5a5c69] mt-[5px]">
              $240,000
            </h1>
          </div>
      ...
      <div className="flex mt-[22px] w-full gap-[30px]">
        <div className="basis-[55%] border bg-white shadow-md cursor-pointer rounded-[4px]">
          ...
          <div className="px-[25px] space-y-[15px] py-[15px]">
            <div>
              <h2>Ready Accounts</h2>
              <Progress percent={30} strokeColor="#E74A3B" />
            </div>
            <div>
              <h2>Female Singups</h2>
